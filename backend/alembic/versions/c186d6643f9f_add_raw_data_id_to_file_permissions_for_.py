...
def upgrade() -> None:
    ...
    op.add_column(
        "file_permissions", sa.Column("raw_data_id", sa.UUID(), nullable=True)
    )

    # Create unique constraint on raw_data_id
    op.create_unique_constraint(
        "unique_raw_data_id_file_permissions", "file_permissions", ["raw_data_id"]
    )

    # Create foreign key to raw_data table
    op.create_foreign_key(
        "fk_file_permissions_raw_data",
        "file_permissions",
        "raw_data",
        ["raw_data_id"],
        ["id"],
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "check_at_least_one_file_type", "file_permissions", type_="check"
    )

    # Drop foreign key
    op.drop_constraint(
        "fk_file_permissions_raw_data", "file_permissions", type_="foreignkey"
    )

    # Drop unique constraint
    op.drop_constraint(
        "unique_raw_data_id_file_permissions", "file_permissions", type_="unique"
    )
