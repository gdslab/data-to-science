...
def upgrade() -> None:
    ...
    op.create_unique_constraint(
        "uq_vector_layers_feature_id", "vector_layers", ["feature_id"]
    )

    # add the new 'vector_layer_feature_id' column to 'data_product_metadata'
    op.add_column(
        "data_product_metadata",
        sa.Column("vector_layer_feature_id", sa.UUID(), nullable=True),
    )

    # copy values from 'vector_layer_id' to 'vector_layer_feature_id'
    op.execute(
        "UPDATE data_product_metadata SET vector_layer_feature_id = vector_layer_id"
    )

    # drop the old foreign key and unique constraint
    op.drop_constraint(
        "data_product_metadata_vector_layer_id_fkey",
        "data_product_metadata",
        type_="foreignkey",
    )
    ...
    op.create_unique_constraint(
        "unique_metadata",
        "data_product_metadata",
        ["data_product_id", "vector_layer_feature_id", "category"],
    )
...
def downgrade() -> None:
    ...
    op.add_column(
        "vector_layers", sa.Column("id", sa.UUID(), autoincrement=False, nullable=True)
    )
    ...
    op.add_column(
        "data_product_metadata", sa.Column("vector_layer_id", sa.UUID(), nullable=True)
    )
    op.execute(
        "UPDATE data_product_metadata SET vector_layer_id = vector_layer_feature_id"
    )
    op.alter_column("data_product_metadata", "vector_layer_id", nullable=True)

    # update constraints
    op.drop_constraint(
        "data_product_metadata_vector_layer_feature_id_fkey",
        "data_product_metadata",
        type_="foreignkey",
    )
    op.create_foreign_key(
        None, "data_product_metadata", "vector_layers", ["vector_layer_id"], ["id"]
    )
    op.drop_constraint("unique_metadata", "data_product_metadata", type_="unique")
    op.create_unique_constraint(
        "unique_metadata",
        "data_product_metadata",
        ["data_product_id", "vector_layer_id", "category"],
    )
