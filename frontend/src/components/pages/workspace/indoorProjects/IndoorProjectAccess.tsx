...
function AccessRoleRadioGroup({
  ...
  memberId,
  uniqueID,
  updateIndoorProjectMemberRole,
}: {
  currentRole: string;
  memberId: string;
  uniqueID: string;
  updateIndoorProjectMemberRole: (
    indoorProjectMemberId: string,
    newRole: string
  ) => void;
}) {
        ...
        onClick={async (e) => {
          ...
          if (currentRole !== role) {
            try {
              ...
              const response: AxiosResponse<IndoorProjectMember> =
                await api.put(
                  `/indoor_projects/${params.indoorProjectId}/members/${memberId}`,
                  payload
                );
              if (response) {
                updateIndoorProjectMemberRole(
...
export default function IndoorProjectAccess() {
  const [error, setError] = useState('');
  ...
  const {
    state: { projectMembers, projectRole },
    dispatch,
  ...
  useEffect(() => {
    ...
    if (projectRole && projectRole === 'owner') {
      setIsLoading(false);
    } else if (projectRole && projectRole !== 'owner') {
      navigate(-1);
    }
    // Still loading if projectRole is undefined
  }, [projectRole, navigate]);

  function updateIndoorProjectMemberRole(
    indoorProjectMemberId: string,
    newRole: string
  ): void {
    if (projectMembers) {
      dispatch({
        ...
        payload: projectMembers.map((indoorProjectMember) => {
          if (indoorProjectMember.id === indoorProjectMemberId) {
            return { ...indoorProjectMember, role: newRole };
          } else {
            return indoorProjectMember;
          }
        }),
      });
    }
  ...
  async function removeIndoorProjectMember(
    memberId: string,
    indoorProjectMembers: IndoorProjectMember[]
  ) {
    try {
      ...
      if (response) {
        dispatch({
          type: 'SET_PROJECT_MEMBERS',
          payload: indoorProjectMembers.filter(({ id }) => id !== memberId),
        });
      }
    } catch (err) {
      if (isAxiosError(err)) {
  ...
  if (isLoading) {
    ...
    return <div className="p-4">Unable to find indoor project members</div>;
  } else {
    return (
      <div className="flex flex-col p-4">
          ...
          <div className="grid grid-rows-3 gap-1.5">
            <div className="grid grid-cols-6 gap-4">
              ...
              <span className="col-span-5">
                Can create, update, view, and remove indoor project data.
              </span>
            </div>
            <div className="grid grid-cols-6 gap-4">
              <span className="col-span-1 font-semibold text-slate-700">
              ...
              <span className="col-span-5">
                Can create, update, and view indoor project data.
              </span>
            </div>
            <div className="grid grid-cols-6 gap-4">
              <span className="col-span-1 font-semibold text-slate-700">
              ...
              <span className="col-span-5">Can view indoor project data.</span>
            </div>
          </div>
        </div>
                    ...
                    .map(({ id, full_name, email, profile_url, role }) => ({
                      ...
                      values: [
                        ...
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            removeIndoorProjectMember(id, projectMembers)
                          }
                        >
                          Remove
                        </button>,
        ...
        <div className="mt-4">
          <h3>Find new indoor project members</h3>
              ...
              <Button
                ...
                onClick={(e) => {
                  ...
                  if (selectedMembers.length > 0) {
                    api
                      .post(
                        `/indoor_projects/${params.indoorProjectId}/members/multi`,
                        selectedMembers.map(({ id }) => id)
                      )
                      .then(
                        (response: AxiosResponse<IndoorProjectMember[]>) => {
                          dispatch({
                            type: 'SET_PROJECT_MEMBERS',
