...
def matching_elements_two_lists(first_list, second_list):
    ...
    if len(not_matching_elements) > 0:
        print(
            "ATTENTION! The following columns do not exist in the data frame:",
...
def create_new_template(
    config_file_name,
    item_TEMPLATE_INPUT,
    item_COLUMNS_TEMPLATE,
    item_NEW_COLUMNS,
    item_SAMPLES_PER_PLOT,
    item_SAMPLE_IDENTIFIER,
    ...
    template_columns_lower = [
        re.sub(r"\s+", "_", s.strip().lower()) for s in template_columns
    ]
    ...
    second_list = [
        re.sub(r"\s+", "_", col.strip().lower()) for col in base_template.columns
    ]
    ...
    number = len(
        base_template_subset.columns
    )  # it will be used to get the dataframe position of new columns
    ...
    for key_column in new_columns:
        ...
        base_template_subset.insert(
            number, key_column_lower, new_columns.get(key_column)
        )
    ...
    times_repeat_row = len(
        [
            value
            for value in get_yml_item_value(
                config_file_name, item_SAMPLES_PER_PLOT
            ).values()
        ][0]
    )
    base_template_subset_rep = base_template_subset.loc[
        base_template_subset.index.repeat(times_repeat_row)
    ]
    ...
    name_column_sample_name = [
        value
        for value in get_yml_item_value(config_file_name, item_SAMPLES_PER_PLOT).keys()
    ][0].lower()
    ...
    list_columns_id = [
        ...
        for x in [
            value
            for value in get_yml_item_value(
                config_file_name, item_SAMPLE_IDENTIFIER
            ).values()
        ][0]
    ]
    ...
    name_column_id = [
        value
        for value in get_yml_item_value(config_file_name, item_SAMPLE_IDENTIFIER).keys()
    ][0].lower()
    ...
    base_template_subset_rep[name_column_id] = (
        base_template_subset_rep[list_columns_id_select]
        .astype(str)
        .agg("_".join, axis=1)
    )
    ...
    list_columns_fileName_lower = [
        re.sub(r"\s+", "_", s.strip().lower()) for s in list_columns_fileName
    ]
    ...
    output_file_name = (
        base_template_subset_rep[list_columns_fileName_select]
        .astype(str)
        .add("_")
        .sum(axis=1)
        .unique()[0][:-1]
